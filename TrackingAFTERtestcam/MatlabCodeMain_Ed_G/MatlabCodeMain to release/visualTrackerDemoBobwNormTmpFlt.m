function visualTrackerDemoBobwNormTmpFlt
clear all;
vidobj = openAxisCam;
...
TargetXMLpath = 'C:\wamp\www';
MaxPeopleAllowedThr =  15; 
...
while 1
    ...
    %Salida = imfilter(Diferencia, flth);
...
%     end
    ...
    if ~isempty(mx) % ~isempty(Centres)
        ...
        for m=1:Npts
            ...
            if Display_Output
                ...
                %if( (~isempty(Centres_Prev)) &  (~isempty(Centres)) & ( speed <= 10) )
                if( (~isempty(Centres_Prev)) &  (~isempty(Centres)) & ( size(Centres_Prev,1) < MaxPeopleAllowedThr )  & ( size(Centres,1) < 20 ) )
                %if( (~isempty(Centres_Prev)) &  (~isempty(Centres))  )
    ...
    im = drawBox(im, Centres);
    
    imwrite( im ,  strcat(TargetXMLpath, '\currentimage', '.jpg') );
