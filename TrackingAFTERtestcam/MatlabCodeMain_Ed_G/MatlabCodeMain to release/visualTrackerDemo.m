...
Sq_L = 20; % 5; % Length of drawn squares
...
while 1
    ...
    if ~isempty(mx) % ~isempty(Centres)
        ...
        for m=1:Npts
            ...
            if Display_Output
                ...
                line( [Centres(m,1)-Sq_L Centres(m,1)-Sq_L], [Centres(m,2)-Sq_L  Centres(m,2)+Sq_L ], 'Color', 'r', 'LineWidth',2)
                line( [Centres(m,1)+Sq_L Centres(m,1)+Sq_L], [Centres(m,2)-Sq_L  Centres(m,2)+Sq_L ], 'Color', 'r', 'LineWidth',2)
                line( [Centres(m,1)-Sq_L Centres(m,1)+Sq_L], [Centres(m,2)-Sq_L  Centres(m,2)-Sq_L ], 'Color', 'r', 'LineWidth',2)
                line( [Centres(m,1)-Sq_L Centres(m,1)+Sq_L], [Centres(m,2)+Sq_L  Centres(m,2)+Sq_L ], 'Color', 'r', 'LineWidth',2)
    ...
    %imwrite( im_orig ,  strcat(TargetXMLpath, '\current_image', '.jpg') );
